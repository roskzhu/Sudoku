...
# find an empty spot in the board, denoted by 0 or "_" *******
def find_empty(brd):
    for i in range(len(brd)):
        # check for the length of each row
        for j in range(len(brd[0])):
            # check if that position is 0 *********or _ 
            if brd[i][j] == 0:
                # return (row, column), like (y, x) *******change to column row
                return (i, j) 
...
def valid(brd, num, posn):
    ...
    for i in range(sbsqr_y*3, sbsqr_x*3 + 3): # ***** (y, x)
